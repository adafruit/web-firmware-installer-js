import{html}from"https://cdn.jsdelivr.net/npm/lit-html/+esm";import{map}from"https://cdn.jsdelivr.net/npm/lit-html/directives/map/+esm";import*as toml from"https://cdn.jsdelivr.net/npm/iarna-toml-esm@3.0.5/+esm";import*as zip from"https://cdn.jsdelivr.net/npm/@zip.js/zip.js@2.6.65/+esm";import{REPL}from"https://cdn.jsdelivr.net/gh/adafruit/circuitpython-repl-js@3.2.1/repl.js";import{InstallButton,ESP_ROM_BAUD,NotRomBootloaderError}from"./base_installer.min.js";let PREFERRED_BAUDRATE=921600,COPY_CHUNK_SIZE=65536,DEFAULT_RELEASE_LATEST=!1,BOARD_DEFS="https://adafruit-circuit-python.s3.amazonaws.com/esp32_boards.json",CSS_DIALOG_CLASS="cp-installer-dialog",attrMap={bootloader:"bootloaderUrl",uf2file:"uf2FileUrl",binfile:"binFileUrl"};class CPInstallButton extends InstallButton{constructor(){super(),this.releaseVersion="[version]",this.boardName="ESP32-based device",this.boardIds=null,this.selectedBoardId=null,this.bootloaderUrl=null,this.boardDefs=null,this.uf2FileUrl=null,this.binFileUrl=null,this.releaseVersion=0,this.chipFamily=null,this.dialogCssClass=CSS_DIALOG_CLASS,this.dialogs={...this.dialogs,...this.cpDialogs},this.bootDriveHandle=null,this.circuitpyDriveHandle=null,this._bootDriveName=null,this._serialPortName=null,this.replSerialDevice=null,this.repl=null,this.fileCache=[],this.reader=null,this.writer=null,this.tomlSettings=null,this.init()}static get observedAttributes(){return Object.keys(attrMap)}parseVersion(e){var t={},e=e.match(/(\d+)\.(\d+)\.(\d+)(?:-([a-z]+)\.(\d+))?/);return e&&4<=e.length&&(t.major=e[1],t.minor=e[2],t.patch=e[3],e[4]&&e[5]?(t.suffix=e[4],t.suffixVersion=e[5]):(t.suffix="stable",t.suffixVersion=0)),t}sortReleases(e){let r=["major","minor","patch","suffix","suffixVersion"];return e.sort((e,t)=>{var i,s=this.parseVersion(e.version),a=this.parseVersion(t.version);for(i of r){if(s[i]<a[i])return-1;if(s[i]>a[i])return 1}return 0}),e}async connectedCallback(){var e=await fetch(BOARD_DEFS),e=(this.boardDefs=await e.json(),this.getAttribute("boardid"));e&&0!==e.trim().length?this.boardIds=e.split(","):this.boardIds=Object.keys(this.boardDefs),1===this.boardIds.length&&(this.selectedBoardId=this.boardIds[0]),this.getAttribute("version")&&(this.releaseVersion=this.getAttribute("version")),super.connectedCallback()}async loadBoard(e){let t=null;if(Object.keys(this.boardDefs).includes(e)){e=this.boardDefs[e],e=(this.chipFamily=e.chipfamily,e.name&&(this.boardName=e.name),e.bootloader&&(this.bootloaderUrl=this.updateBinaryUrl(e.bootloader)),this.sortReleases(e.releases));if(this.releaseVersion)for(var i of e)if(i.version==this.releaseVersion){t=i;break}t||(t=DEFAULT_RELEASE_LATEST?e[e.length-1]:e[0],this.releaseVersion=t.version),t.uf2file&&!this.uf2FileUrl&&(this.uf2FileUrl=this.updateBinaryUrl(t.uf2file)),t.binfile&&!this.binFileUrl&&(this.binFileUrl=this.updateBinaryUrl(t.binfile))}this.getAttribute("chipfamily")&&(this.chipFamily=this.getAttribute("chipfamily")),this.getAttribute("boardname")&&(this.boardName=this.getAttribute("boardname")),this.menuTitle="CircuitPython Installer for "+this.boardName}attributeChangedCallback(e,t,i){this[attrMap[e]]=i?this.updateBinaryUrl(i):null}updateBinaryUrl(e){return e=e&&e.replace("https://downloads.circuitpython.org/","https://adafruit-circuit-python.s3.amazonaws.com/")}flows={uf2FullProgram:{label:"Full CircuitPython [version] Install",steps:[this.stepWelcome,this.stepFsapiCheck,this.stepSerialConnect,this.stepConfirm,this.stepEraseAll,this.stepBootloader,this.stepSelectBootDrive,this.stepCopyUf2,this.stepSelectCpyDrive,this.stepCredentials,this.stepSuccess],isEnabled:async()=>this.hasNativeUsb()&&!!this.bootloaderUrl&&!!this.uf2FileUrl},binFullProgram:{label:"Full CircuitPython [version] Install",steps:[this.stepWelcome,this.stepSerialConnect,this.stepConfirm,this.stepEraseAll,this.stepFlashBin,this.stepSetupRepl,this.stepCredentials,this.stepSuccess],isEnabled:async()=>!this.hasNativeUsb()&&!!this.binFileUrl},uf2Only:{label:"Install CircuitPython [version] UF2 Only",steps:[this.stepWelcome,this.stepSelectBootDrive,this.stepCopyUf2,this.stepSelectCpyDrive,this.stepCredentials,this.stepSuccess],isEnabled:async()=>this.hasNativeUsb()&&!!this.uf2FileUrl&&this.hasFileSystemAccess},binOnly:{label:"Install CircuitPython [version] Bin Only",steps:[this.stepWelcome,this.stepSerialConnect,this.stepConfirm,this.stepEraseAll,this.stepFlashBin,this.stepSuccess],isEnabled:async()=>!!this.binFileUrl},bootloaderOnly:{label:"Install Bootloader Only",steps:[this.stepWelcome,this.stepSerialConnect,this.stepConfirm,this.stepEraseAll,this.stepBootloader,this.stepSuccess],isEnabled:async()=>this.hasNativeUsb()&&!!this.bootloaderUrl},credentialsOnlyRepl:{label:"Update WiFi credentials",steps:[this.stepWelcome,this.stepSetupRepl,this.stepCredentials,this.stepSuccess],isEnabled:async()=>!this.hasNativeUsb()},credentialsOnlyDrive:{label:"Update WiFi credentials",steps:[this.stepWelcome,this.stepSelectCpyDrive,this.stepCredentials,this.stepSuccess],isEnabled:async()=>this.hasNativeUsb()&&this.hasFileSystemAccess}};cpDialogs={boardSelect:{closeable:!0,template:i=>html`
            ...
            `,buttons:[this.previousButton,{label:"Next",onClick:this.nextStep,isEnabled:async()=>this.currentStep<this.currentFlow.steps.length-1&&this.connected==this.connectionStates.CONNECTED,onUpdate:async e=>{this.currentDialogElement.querySelector("#butConnect").innerText=this.connected}}]},notRomBootloader:{closeable:!0,template:e=>html`
                <h3>Not in ROM Bootloader mode</h3>
                ${(e&&e.message?e.message.split("\n\n"):[]).filter(e=>0<e.trim().length).map(e=>html`<p>${e}</p>`)}
            `,buttons:[{label:"OK",onClick:async e=>{this.closeDialog(),this.currentFlow&&"function"==typeof this.currentFlow.steps[this.currentStep]&&await this.currentFlow.steps[this.currentStep].bind(this)()}}]},confirm:{template:e=>html`
                <h3>Erase Flash</h3>
                <p>Now, optionally, erase everything on the ${e.boardName}.</p>
            `,buttons:[this.previousButton,{label:"Skip Erase",onClick:async e=>{confirm("Skipping the erase step may cause issues and is not recommended. Continue?")&&await this.advanceSteps(2)}},{label:"Continue",onClick:this.nextStep}]},fsapiUnavailable:{closeable:!0,template:e=>html`
            ...
            `,buttons:[{label:"Use Another Browser",onClick:async e=>{this.closeDialog()}},{label:"Install .bin Instead",onClick:this.installBinInsteadHandler,onUpdate:async e=>{e.target.style.display=this.binFileUrl?"":"none"}},{label:"Continue Manually",onClick:this.continueManuallyHandler}]},manualBootCopy:{closeable:!0,template:e=>html`
                <h3>Copy CircuitPython onto the ${e.drivename} drive</h3>
                <ol>
                    <li>
                        <p>
                            <strong>Reset your board.</strong> Press the
                            RESET button once. A new drive named
                            <code>${e.drivename}</code> should appear
                    ...
                    <li>
                        <p>
                            ...
                            <a href="${e.uf2FileUrl}" download target="_blank" rel="noopener">
                                Download <code>${e.uf2FileName}</code>
                            </a>
                        </p>
                    </li>
                    <li>
                        <p>
                            ...
                            <code>${e.drivename}</code> drive.</strong>
            ...
            `,buttons:[this.previousButton,this.nextButton]},manualCircuitPyWait:{closeable:!0,template:e=>html`
            ...
            `,buttons:[this.previousButton,this.nextButton]},manualSuccess:{closeable:!0,template:e=>html`
...
CIRCUITPY_WEB_API_PORT = 80</pre>
            ...
            `,buttons:[this.closeButton]},bootDriveSelect:{closeable:!0,template:e=>html`
